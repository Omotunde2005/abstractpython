...
This API is so powerful that you may not need to specify the IP address of your current location. It automatically detects the IP address the query came from.
...
The code above uses the IP address in the previous response data. Below is the response data for the code above:
